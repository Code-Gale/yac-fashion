    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 lg:mb-14">
          <div>
            <p className="text-[#c9a84c] uppercase tracking-[0.2em] text-xs font-medium mb-2">Just landed</p>
            ...
            <p className="text-[#6b7280] text-base mt-2 max-w-xl">
              Fresh styles added to the collection this week
            </p>
          </div>
          <Link
        ...
        <div
          className={cn(
            'flex gap-4 overflow-x-auto scroll-smooth snap-x snap-mandatory scrollbar-hide -mx-6 px-6 lg:mx-0 lg:px-0 lg:grid lg:grid-cols-4 lg:gap-8 lg:overflow-visible',
            visible && 'opacity-100'
          )}
        ...
        >
          {products.map((p, i) => (
            <div
              key={p._id}
              className={cn(
                'flex-shrink-0 w-[calc(100%-2rem)] sm:w-72 snap-center lg:flex-shrink lg:w-auto',
