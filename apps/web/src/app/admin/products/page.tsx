  ...
  const columns = [
    ...
    { key: 'actions', label: '', width: '100px', render: (r: any) => (
      <div className="flex gap-1">
        ...
        <button type="button" onClick={(e) => { e.stopPropagation(); api.delete(`/admin/products/${r._id}`).then(() => { toast('Deleted'); fetchProducts(); }).catch(() => toast('Failed', 'error')); }} className="admin-btn admin-btn-ghost !min-h-[36px] !px-2 hover:!text-[var(--admin-error)]" aria-label="Delete">
      ...
      <AdminPanel>
        <AdminTable
          ...
          mobileCardRender={(r, i) => (
            <AdminMobileCard href={`/admin/products/${r._id}/edit`} index={i}>
